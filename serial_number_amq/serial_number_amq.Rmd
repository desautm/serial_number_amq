...
Nous trouvons en \cite{Goodman1954} une application concrète de l'utilisation de l'utilisation de nos estimateurs. On nous indique que le département des sciences sociales de l'Université de Chicago utilisaient de l'équipement (bureaux, chaises, etc.) sur lesquels étaient placés des numéros de série.
...
```
Les numéros de série de `r length(equip)` pièces d'équipement ont été observées. Les `r length(equip)` numéros de série étaient:
```{r, echo=F}
equip
```
...
N5(equip)
```
Les équipement ayant été achetés entre 1928 et 1934, l'auteur de \cite{Goodman1954} indique qu'il lui a été particulièrement difficile d'obtenir la véritable valeur du nombre d'équipements achetés. Après plusieurs jours et de nombreuses questions aux personnes concernés, les archives pertinentes ont été trouvées et le nombre $N$ de pièces d'équipements était de $2885$.

# Code `R` \label{code}

Pour la lectrice ou le lecteur intéressé, vous trouverez le code `R` complet qui a servi à produire les diverses figures de cet article.
